...
{
    use SingletonTrait;
    public static string $prefix = "§b§l[EconomyAPI] §r§7";
    ...
    public function onEnable():void
    {
        ...
        // Provider set
        $this->provider = match (strtolower($this->getConfig()->get("provider"))) {
            "yaml" => new YamlProvider($this),
            default => throw new \UnexpectedValueException("Invalid database was given"),
        };
    ...
    public function myMoney($player): float|bool
    {
        $player = strtolower($player instanceof Player ? $player->getName() : $player);
    ...
    public function getRank($player): bool|int
    {
        $player = strtolower($player instanceof Player ? $player->getName() : $player);
    ...
    public function getPlayerByRank(int $rank): bool|string
    {
